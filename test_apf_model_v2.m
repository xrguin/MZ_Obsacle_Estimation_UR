%% Test APF Model - Version 2 (Normalized)
% This version uses the normalized model: Sliced_strict_apf_net1_norm.mat
% The model was trained with normalized input/output data, so this script:
% 1. Normalizes input data before prediction
% 2. Denormalizes output data after prediction
% Based on provided code structure using strict_apf_ally_turn_1.mat
...
multiSliceNet_mw9 = trainedModel.net;  % Extract the network from the struct
...
fprintf('Loaded normalization parameters from model:\n');
fprintf('Input normalization - Mean: [%.4f, %.4f, %.4f], Std: [%.4f, %.4f, %.4f]\n', ...
    muX_train, sigmaX_train);
fprintf('Output normalization - Mean: [%.4f, %.4f], Std: [%.4f, %.4f]\n', ...
...
for t = 1:numPredictionTimeSteps-1
  ...
  %  if predictionStarted
      ...
      XTrajTest{j} = Xmoving_window_norm;  % Store normalized input
      TTrajTest{j} = Tmoving_window;
      prediction_time_list(end+1) = t;
      
      % Predict with normalized input
      Y_norm(t, :) = predict(multiSliceNet_mw9, XTrajTest{j});
...
for i = 1:length(bad_j_idx)
    j_idx = bad_j_idx(i);
    xwindow_norm = XTrajTest{j_idx};
    % Denormalize for visualization
    xwindow = xwindow_norm .* sigmaX_train + muX_train;
